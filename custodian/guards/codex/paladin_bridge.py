...
def wire_git_helper(host: str, ref: str) -> tuple[bool, str]:
    ...
    """
    paladin_path = shutil.which("paladin")
    if paladin_path is None:
        return False, "paladin CLI not on PATH"
    try:
        # Use the resolved path, not the bare name: on Windows the CLI is a
        # `paladin.cmd` launcher, and unlike shutil.which (PATHEXT-aware),
        # subprocess with a bare command name is not -- it would raise
        # FileNotFoundError here even though the probe above just confirmed
        # paladin is genuinely on PATH.
        proc = subprocess.run(
            [paladin_path, "git-setup", host, ref],
