...
def _gates_for(*, tool: str, kind: str, arguments: dict[str, Any],
               workspace: str, paths: list[str]) -> list[str]:
    gates: list[str] = []
    if kind == "read":
    ...
    if normalized in {"shell", "bash", "terminal", "shell-exec", "exec", "exec_command"}:
        gates.append("shell")
    if kind == "network":
        gates.append("network")
    if kind == "credential":
        gates.append("credentials")
    if kind == "destructive":
