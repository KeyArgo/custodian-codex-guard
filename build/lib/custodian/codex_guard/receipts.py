    ...
    def append(self, decision: dict[str, Any], *, tool: str, session_id: str,
        ...
        with self._lock, _process_lock(self.lock_path):
            ...
            if isinstance(gates, list):
                body["gates"] = [
                    {
                        "gate": str(item.get("gate", ""))[:64],
                        "mode": str(item.get("mode", ""))[:16],
                        "rule_id": str(item.get("rule_id", ""))[:128],
                        "scope": str(item.get("scope", ""))[:32],
                    }
        ...
        try:
            ...
            ledger.append(LedgerEvent(
                ...
                metadata={
                    ...
                    "gates": ",".join(
                        f"{gate['gate']}:{gate['mode']}:{gate['rule_id']}:{gate['scope']}"
